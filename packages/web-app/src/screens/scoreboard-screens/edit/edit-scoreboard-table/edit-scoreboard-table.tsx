...
const ScoreboardTables: React.FC<IScoresTableProps> = ({ riderAllocations, eventId, noProgressing }) => {
  ...
  const getUpdatedPosition = (formValues: IScoreRunFormValues): IScoreFormPosition => {
    ...
    const copyRiderAllocations = _.cloneDeep(scoresTableData.map(({ riderAllocation }) => riderAllocation));
  ...
  return (
    <>
      ...
      <ScoreboardDataTable
        tableData={_.orderBy(scoresTableData, (row) => row.riderAllocation.startOrder)}
