...
import { CloudFrontWebDistribution, ViewerCertificate, OriginAccessIdentity, Behavior, SSLMethod, SecurityPolicyProtocol } from '@aws-cdk/aws-cloudfront';
...
export interface SPADeployConfig {
  ...
  readonly cloudfrontExtraDistibutionPaths?: string[];
...
export class SPADeploy extends cdk.Construct {
  ...
  public createSiteWithCloudfront(config: SPADeployConfig): SPADeploymentWithCloudFront {
    ...
    const distributionPaths = ['/', `/${config.indexDoc}`, ...config.cloudfrontExtraDistibutionPaths];
